...
export function parseIndexSitemap(html) {
    const bodyMatch = /<body[^>]*>([\s\S]*?)<\/body>/i.exec(html);
    const content = bodyMatch?.[1] ?? html;
    const rootUl = findFirstUl(content);
    if (!rootUl) {
...
function extractBalancedTagContent(html, start, tag) {
    let depth = 1;
    let index = start;
...
function splitDirectLiSegments(ulInner) {
    const segments = [];
    let ulDepth = 0;
    let liStart = -1;
    let index = 0;
...
function parseUlChildren(ulInner) {
    const nodes = [];
    let pos = 0;
...
function parseUnclosedLiUlChildren(ulInner) {
    const nodes = [];
...
function parseIndexUlChildren(ulInner) {
    const nodes = [];
    for (const segment of splitDirectLiSegments(ulInner)) {
...
function parseTocListItemSegment(segment) {
    const ulMatch = /<ul\b/i.exec(segment);
    const beforeUl = ulMatch ? segment.slice(0, ulMatch.index) : segment;
...
function parseListItem(liInner) {
    const objectMatch = /<object\b[^>]*type\s*=\s*["']text\/sitemap["'][^>]*>([\s\S]*?)<\/object>/i.exec(liInner);
    let name = "";
    let local;
    if (objectMatch?.[1]) {
...
function parseIndexObjectParams(liInner) {
    const objectMatch = /<object\b[^>]*type\s*=\s*["']text\/sitemap["'][^>]*>([\s\S]*?)<\/object>/i.exec(liInner);
    if (!objectMatch?.[1]) {
        return [];
    }
...
function extractAllParams(objectBody) {
    const params = [];
...
function extractParam(objectBody, paramName) {
    const re = new RegExp(`<param\\b[^>]*name\\s*=\\s*["']${paramName}["'][^>]*value\\s*=\\s*["']([^"']*)["']`, "i");
    const match = re.exec(objectBody);
    if (match?.[1]) {
        return decodeEntities(match[1].trim());
    }
    const reAlt = new RegExp(`<param\\b[^>]*value\\s*=\\s*["']([^"']*)["'][^>]*name\\s*=\\s*["']${paramName}["']`, "i");
...
export function mapSitemapToToc(nodes) {
...
export function mapSitemapToIndex(nodes) {
    return nodes.map((node) => ({
        name: node.name,
        local: node.local,
...
export function countSitemapNodes(nodes) {
    return nodes.reduce((total, node) => total + 1 + countSitemapNodes(node.children), 0);
}
export function maxSitemapDepth(nodes, depth = 0) {
    if (nodes.length === 0) {
        return depth;
    }
