...
import { CONVERTER_VERSION, chmTopicToRoute, classifyChmPath, normalizeChmPath, resolveChmHref, routeToMarkdownPath, toDiskPath, } from "@chm-md/shared";
...
import { collapseChromeStripWarnings, preprocessTables } from "./tables.js";
function createTurndown(profile) {
...
function removeCommentsAndScripts(document) {
    const comments = [];
...
function walkDocumentNodes(root, visitor) {
    const stack = [root];
...
function extractLinkedStyles(document) {
...
function extractOriginalIds(document) {
    const ids = [];
...
function rewriteAssetHref(href, topicPath, assets, downloads, warnings) {
...
function rewriteTopicHref(href, topicPath, topicRoutes, outboundLinks, topicFragmentMaps) {
    const resolved = resolveChmHref(topicPath, href);
    if (resolved.startsWith("#") || resolved.includes("://") || resolved.startsWith("mailto:")) {
...
function buildTopicFragmentMaps(htmlTopics, files) {
    const maps = new Map();
...
function preprocessHtml(html, topicPath, assets, downloads, topicRoutes, topicFragmentMaps, warnings, tableChrome = "strip") {
    const { document } = parseHTML(html);
    const outboundLinks = [];
    ...
    for (const anchor of document.querySelectorAll("a[href]")) {
        ...
        const kind = classifyChmPath(normalized, false);
        if (kind === "html") {
            anchor.setAttribute("href", rewriteTopicHref(href, topicPath, topicRoutes, outboundLinks, topicFragmentMaps));
            continue;
        }
        anchor.setAttribute("href", rewriteAssetHref(href, topicPath, assets, downloads, warnings));
    }
    for (const image of document.querySelectorAll("img[src]")) {
        ...
        image.setAttribute("src", rewriteAssetHref(src, topicPath, assets, downloads, warnings));
    }
...
function buildAssetsAndDownloads(bundle, files) {
    const assets = [];
    const downloads = [];
...
function collectHtmlTopics(bundle) {
    return bundle.entries
        .filter((entry) => entry.kind === "html" && entry.size > 0)
        .map((entry) => entry.path);
}
function mapTocToNav(nodes, topicRoutes) {
    return nodes.map((node) => {
        const navNode = { text: node.name };
        if (node.local) {
...
function flattenIndex(nodes, topicRoutes) {
    const entries = [];
    for (const node of nodes) {
        const entry = { keyword: node.name };
        if (node.local) {
...
function collectKeywordsForTopic(topicPath, bundle) {
...
export function convertBundle(options) {
    const warnings = [];
    const htmlTopics = collectHtmlTopics(options.bundle);
    const topicRoutes = new Map(htmlTopics.map((topic) => [topic, chmTopicToRoute(topic)]));
    ...
    for (const [index, topicPath] of htmlTopics.entries()) {
        ...
        const processed = preprocessHtml(html, topicPath, assetMap, downloadMap, topicRoutes, topicFragmentMaps, warnings, tableChrome);
...
export async function writeProject(options) {
    ...
    await writeFile(join(outputDir, "warnings.json"), JSON.stringify(project.warnings, null, 2), "utf8");
    await writeFile(join(outputDir, "assets.json"), JSON.stringify(project.assets, null, 2), "utf8");
    await writeFile(join(outputDir, "downloads.json"), JSON.stringify(project.downloads, null, 2), "utf8");
...
function countWarningCodes(warnings, codes) {
    const counts = {};
...
function countWarningsByPrefix(warnings, prefix) {
    const counts = {};
    for (const warning of warnings) {
