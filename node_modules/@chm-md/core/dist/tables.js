...
function classifyReason(table, kind) {
    if (kind === "spanned") {
...
function buildTableWarningDetails(table, kind, action, reason) {
    return {
        kind,
        action,
        reason,
...
function isHeaderOnlyNoteTable(table, rows) {
    if (rows.length !== 1) {
        return false;
    }
    const row = rows[0];
    ...
    if (tdCells.length === 1 && thCells.length === 0) {
        ...
        const hasBoldOnly = tdCells[0]?.querySelector("b, strong") !== null &&
            (tdCells[0]?.textContent?.trim().length ?? 0) > 0;
        return text.length > 0 && hasBoldOnly;
    }
    return false;
}
function extractNoteText(table) {
...
function extractFlattenedText(table) {
    const paragraphs = [...table.querySelectorAll("td, th")]
        .map((cell) => cell.textContent?.trim() ?? "")
        .filter((text) => text.length > 0);
    return paragraphs.join("\n\n");
}
function isChromeOnlyContent(table) {
...
function expandTableToGrid(table) {
    ...
    for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
        ...
        for (const cell of [...row.querySelectorAll(":scope > th, :scope > td")]) {
            ...
            const gridCell = { tag, text: cellPlainText(cell) };
    ...
    return grid.map((row) => row.map((cell) => cell ?? { tag: "td", text: "" }));
}
function buildSimpleTableHtml(grid) {
    if (grid.length === 0) {
        return "";
    }
...
function flattenTableForGfm(table) {
    const parts = [];
...
function gfmFlattenResult(table, kind, sourcePath, warningCode, warningMessage, reason) {
...
export function processTable(table, sourcePath, chromeMode = "strip") {
    ...
    if (kind === "layout-chrome") {
        ...
        return gfmFlattenResult(table, kind, sourcePath, chromeMode === "preserve" ? "layout-table-flattened" : "layout-table-flattened", "Layout/chrome table flattened to GFM table", reason);
    }
    if (kind === "spanned") {
        return gfmFlattenResult(table, kind, sourcePath, "table-span-flattened", "Table with colspan/rowspan flattened to GFM table", reason);
    }
    if (kind === "nested") {
        return gfmFlattenResult(table, kind, sourcePath, "table-nested-flattened", "Nested table flattened to sequential GFM tables", reason);
    }
    return gfmFlattenResult(table, kind, sourcePath, "layout-table-flattened", "Table flattened to GFM table", reason);
}
/** Collapse identical layout-table-stripped warnings into summary entries. */
export function collapseChromeStripWarnings(warnings) {
    const collapsed = [];
...
function replaceTableNode(table, replacementHtml) {
...
export function preprocessTables(document, sourcePath, warnings, chromeMode = "strip") {
