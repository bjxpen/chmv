...
export function escapeMarkdownProse(markdown) {
    ...
    return segments
        .map((segment) => segment.type === "prose" ? escapeProseContent(segment.content) : segment.content)
        .join("");
}
/** @deprecated Prefer escapeMarkdownProse. */
export function escapeAngleBracketsInProse(markdown) {
 ...
 */
function escapeProseContent(prose) {
...
function findMarkdownLinks(prose) {
    const links = [];
    let index = 0;
...
function readDestination(prose, start) {
...
function findMatchingOpenBracket(prose, closeIndex) {
    let depth = 0;
...
export function splitMarkdownSegments(markdown) {
    ...
    while (index < markdown.length) {
        ...
        const nextSpecial = Math.min(nextFence === -1 ? Number.POSITIVE_INFINITY : nextFence, nextHtml === -1 ? Number.POSITIVE_INFINITY : nextHtml, nextInline === -1 ? Number.POSITIVE_INFINITY : nextInline);
...
function findInlineCodeEnd(markdown, startIndex) {
...
function findNextInlineCodeStart(markdown, fromIndex) {
...
function findHtmlBlockStart(markdown, fromIndex) {
...
function findHtmlBlockEnd(markdown, startIndex) {
