...
export function preprocessCodeBlocks(document) {
...
export function preprocessCodeSnippets(document) {
    for (const container of [...document.querySelectorAll(".CodeSnippetContainer")]) {
        ...
        for (const codeDiv of container.querySelectorAll(".CodeSnippetContainerCode")) {
            ...
            blocks.push(`<p><strong>${escapeHtml(label)}</strong></p><pre><code class="language-${fence}">${escapeHtml(codeText)}</code></pre>`);
        }
        if (blocks.length === 0) {
            container.remove();
            continue;
...
function preprocessStandalonePreElements(document) {
...
function preprocessMonospaceElements(document) {
    const candidates = [
...
function isMonospaceElement(element) {
...
function replaceWithCodeElement(document, element, codeText) {
...
export function neutralizeJavascriptLinks(document, sourcePath, warnings) {
