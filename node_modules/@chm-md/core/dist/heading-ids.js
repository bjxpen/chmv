...
export function assignHeadingIds(document) {
    const assignments = [];
...
export function postProcessMarkdown(markdown, profile, options) {
    let body = markdown;
    if (profile.expandTabs) {
        const spaces = " ".repeat(profile.tabWidth);
        body = splitMarkdownSegments(body)
            .map((segment) => segment.type === "fenced-code" || segment.type === "inline-code"
            ? segment.content
